...
if uploaded_file is not None:
  ...
  if st.button("Iniciar Processamento"):
    ...
    tempo_inicio_2 = time.time() # Variável renomeada para local
    image_gray = cv2.cvtColor(image_cv, cv2.COLOR_BGR2GRAY)
    image_gray_rgb = cv2.cvtColor(image_gray, cv2.COLOR_GRAY2RGB)
    st.image(Image.fromarray(image_gray_rgb), caption="Imagem em Escala de Cinza", use_container_width=True)
    tempo_fim_2 = time.time() # Variável renomeada para local
    st.info(f"Tempo: {tempo_fim_2 - tempo_inicio_2:.3f} segundos")
    ...
    tempo_inicio_ocr = time.time() # Variável renomeada para local
    ...
    tempo_fim_ocr = time.time() # Variável renomeada para local
    ...
    st.image(Image.fromarray(image_with_bbox_rgb), caption="Imagem com Bounding Boxes e Texto Detectado", use_container_width=True)
    # O cálculo do tempo aqui estava usando tempo_inicio/fim da Etapa 2 e Etapa 1
    st.info(f"Tempo de Detecção + Reconhecimento (OCR Core): {tempo_fim_ocr - tempo_inicio_ocr:.3f} segundos")
    ...
    for i, (bbox, text, prob) in enumerate(results_filtered):
      ...
      with col2:
        ...
        if cropped.size > 0:
          ...
          st.image(Image.fromarray(cropped_rgb), caption=f"Região Cortada do Bloco {i+1}", use_container_width=True)
    ...
    st.json({
      ...
      "Tempos por Etapa": {
        # ajustando os logs para as variáveis locais renomeadas
